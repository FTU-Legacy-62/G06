...
.pp-brand .price-word { color: #f8fafc; }
.pp-brand .pilot-word { color: #26e889; }
...
<body>
<div class="header pp-brand"><span class="price-word">Price</span><span class="pilot-word">Pilot</span></div>

<section class="hero">
    <h1 class="pp-brand"><span class="price-word">Price</span><span class="pilot-word">Pilot</span></h1>
    <p>
        <span class="pp-brand"><span class="price-word">Price</span><span class="pilot-word">Pilot</span></span> supports SME mechanical manufacturing businesses in pricing decisions.
        The tool uses historical price-demand data to calculate Elasticity, then applies
    ...
    <div class="buttons">
        ...
        <button class="detail" onclick="openModal()">How PricePilot Works</button>
    </div>
</section>
...
<div class="modal" id="detailModal">
    <div class="modal-content">
        ...
        <h2>How <span class="pp-brand"><span class="price-word">Price</span><span class="pilot-word">Pilot</span></span> Works</h2>

        <p>
        <span class="pp-brand"><span class="price-word">Price</span><span class="pilot-word">Pilot</span></span> uses an <b>Elasticity-based demand model</b>. Price increase is the firm's decision variable,
        ...
        <h3>2. Elasticity from Historical Data</h3>
        <p>
        ...
        <p>
        <b>Definition:</b> Wikipedia describes Monte Carlo methods as computational methods that use repeated random sampling to obtain numerical results.
        In simple terms, instead of assuming that the future will follow one exact scenario, the model tests many plausible versions of the future and looks at the overall pattern of results.
        </p>
        <p>
        In PricePilot, the uncertain parts are the market conditions: market growth, customer budget pressure, material shock impact, and material inflation.
        The user gives a realistic range for each factor because the exact future value is not known in advance.
        </p>
        <p>
        For every possible price increase, PricePilot randomly picks one value from each range, calculates the resulting demand, cost, and profit, then repeats this process many times.
        Each repetition represents one plausible business situation. The web then summarizes these repetitions into expected profit, average demand, demand loss, and the probability of earning less than the current profit.
        </p>
        <p>
        This is useful because the recommendation is not based on one optimistic or pessimistic forecast. It is based on how a price decision performs across many possible market conditions, so the user can compare both potential return and downside risk.
        </p>

        <h3>4. Implementation Plan</h3>
        <p>
        After generating the recommended price range, PricePilot builds an implementation plan to show how the new price can be introduced in practice. The plan uses the suggested range together with expected demand loss, downside risk, historical Elasticity, and estimated material inflation.
        </p>
        <p>
        The implementation plan is guided by Reference Price Theory. This theory explains that customers usually do not evaluate a new price on its own. Instead, they compare it with a reference price, such as a previous quotation, a past purchase price, or the price they expected to pay.
        </p>
        <p>
        Therefore, PricePilot recommends starting with the lower or middle part of the suggested range rather than applying the highest increase to every customer immediately. The new price can first be tested through new quotations, selected repeat customers, or orders where quality, delivery reliability, and technical requirements matter more than the lowest price.
        </p>
    </div>
</div>
...
body {
    ...
    color: var(--text);
    background:
        radial-gradient(circle at 10% 0%, rgba(0,204,102,0.13), transparent 32%),
        radial-gradient(circle at 92% 12%, rgba(56,189,248,0.10), transparent 28%),
        linear-gradient(135deg, #071020 0%, #0f172a 52%, #08111f 100%);
    min-height: 100vh;
}
.topbar {
    height: 64px;
...
.brand-icon {
    ...
    place-items: center;
    color: var(--green-2);
    background: linear-gradient(135deg, rgba(0,204,102,.22), rgba(56,189,248,.08));
    border: 1px solid rgba(0,204,102,.30);
}
.pp-brand .price-word { color: var(--text); }
.pp-brand .pilot-word { color: var(--green-2); }
.topbar a {
    color: var(--green-2);
    text-decoration: none;
    font-weight: 800;
...
.page-icon {
    ...
    color: var(--green-2);
    font-size: 31px;
    background: linear-gradient(135deg, rgba(0,204,102,.18), rgba(56,189,248,.07));
    border: 1px solid rgba(0,204,102,.25);
    box-shadow: 0 18px 45px rgba(0,204,102,.12);
}
h1 {
    margin: 0;
...
.panel {
    background: linear-gradient(180deg, rgba(18, 34, 58, .78), rgba(10, 22, 39, .86));
...
input:focus, select:focus {
    border-color: rgba(0,204,102,.55);
    box-shadow: 0 0 0 4px rgba(0,204,102,.08);
}
select {
    appearance: none;
    background:
...
select option { background: #111827; color: white; padding: 12px; }
...
.dual-slider input[type="range"]::-webkit-slider-runnable-track { height: 6px; background: transparent; }
.dual-slider input[type="range"]::-webkit-slider-thumb {
    ...
    box-shadow: 0 0 0 4px rgba(0,204,102,.16), 0 8px 18px rgba(0,0,0,.35);
    cursor: pointer;
    appearance: none;
    -webkit-appearance: none;
    margin-top: -7px;
...
.mc-option input:checked + span {
    color: #071020;
    background: linear-gradient(135deg, var(--green), var(--green-2));
    border-color: rgba(0,204,102,.70);
    box-shadow: 0 12px 28px rgba(0,204,102,.22);
}
.info-box {
    display: grid;
...
.info-icon {
    ...
    border-radius: 13px;
    color: var(--green-2);
    border: 1px solid rgba(0,204,102,.24);
    background: rgba(0,204,102,.10);
}
.bottom-bar {
    margin-top: 18px;
    display: grid;
...
.run-btn {
    ...
    border: 0;
    border-radius: 16px;
    background: linear-gradient(135deg, #26e889, #00cc66);
    color: #071020;
    font-size: 22px;
    font-weight: 950;
    cursor: pointer;
    box-shadow: 0 18px 46px rgba(0,204,102,.28);
    transition: .2s ease;
}
.run-btn:hover { transform: translateY(-2px); box-shadow: 0 24px 56px rgba(0,204,102,.38); }
@media (max-width: 1200px) {
    .form-layout { grid-template-columns: 1fr; }
    .uncertainty-grid { grid-template-columns: repeat(2,minmax(0,1fr)); }
}
@media (max-width: 760px) {
    .topbar { padding: 0 18px; }
    .shell { width: min(100% - 24px, 1540px); }
    .input-grid, .uncertainty-grid, .bottom-bar { grid-template-columns: 1fr; }
...
<body>
<div class="topbar">
    <div class="brand"><div class="brand-icon">↗</div><span class="pp-brand"><span class="price-word">Price</span><span class="pilot-word">Pilot</span></span> <span style="color:var(--green-2); font-size:16px; margin-left:8px;">Input</span></div>
...
<div class="shell">
    <div class="page-title">
        ...
        <div>
            <h1><span class="pp-brand"><span class="price-word">Price</span><span class="pilot-word">Pilot</span></span> Input</h1>
            <p class="subtitle">Provide cost structure, baseline assumptions, historical data, and uncertainty ranges to run a Monte Carlo pricing simulation.</p>
        </div>
    </div>
    ...
    <form method="POST" action="/run">
        <div class="form-layout">
            <div>
                <section class="panel">
                    <div class="section-head"><div class="section-num">1</div><h2>Monthly Cost Inputs</h2></div>
                    <div class="input-grid">
                        ...
                        <div class="form-group">
                            ...
                            <small>Planned or expected monthly volume.</small>
                        </div>
                        <div class="form-group">
            ...
            <div>
                <section class="panel">
                    <div class="section-head"><div class="section-num">2</div><h2>Business Baseline Inputs</h2></div>
                    <div class="input-grid">
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <div class="range-line">
                                <div class="single-range">
                                    ...
                                    <div class="range-minmax"><span>0%</span><span>50%</span></div>
                                </div>
                                <div class="value-pill"><span id="marginVal">20</span>%</div>
                            </div>
                            <small>Target operating margin on selling price.</small>
                        </div>
                    </div>
                </section>

                <section class="panel">
                    <div class="section-head"><div class="section-num">3</div><h2>Historical Data for Elasticity</h2></div>
                    <div class="input-grid">
                        <div class="form-group">
                            <label>Old selling price (VND/order)</label>
                            <input class="money-input" type="text" name="old_price" value="100,000,000">
                            <small>Previous average selling price.</small>
                        </div>
                        <div class="form-group">
                            <label>New selling price (VND/order)</label>
                            <input class="money-input" type="text" name="new_price_history" value="110,000,000">
                            <small>Newer average selling price.</small>
                        </div>
                        <div class="form-group">
            ...
            <section class="panel market-panel">
                <div class="section-head"><div class="section-num">4</div><h2>Market Uncertainty Ranges</h2></div>
                <p class="hint" style="margin-top:-8px; margin-bottom:18px;">Define the uncertainty bands sampled by Monte Carlo. All ranges below use the technical bounds requested for the model.</p>
                <div class="uncertainty-grid">
                    <div class="range-card">
                        ...
                        <div class="dual-slider">
                            <div class="slider-track-bg"></div><div class="slider-track-fill" id="market_growth_fill"></div>
                            <input type="range" id="market_growth_low" min="-20" max="20" value="0" oninput="updateDualRange('market_growth', '%', this)">
                            <input type="range" id="market_growth_high" min="-20" max="20" value="5" oninput="updateDualRange('market_growth', '%', this)">
                        </div>
                        <div class="range-minmax"><span>-20%</span><span>20%</span></div>
                        <div class="dual-label-row"><div><small>Min</small><span id="market_growth_low_label">0%</span></div><div><small>Max</small><span id="market_growth_high_label">5%</span></div></div>
                    </div>

                    <div class="range-card">
                        <label>Customer budget pressure (%)</label>
                        ...
                        <div class="dual-slider">
                            <div class="slider-track-bg"></div><div class="slider-track-fill" id="budget_pressure_fill"></div>
                            <input type="range" id="budget_pressure_low" min="0" max="20" value="1" oninput="updateDualRange('budget_pressure', '%', this)">
                            <input type="range" id="budget_pressure_high" min="0" max="20" value="5" oninput="updateDualRange('budget_pressure', '%', this)">
                        </div>
                        <div class="range-minmax"><span>0%</span><span>20%</span></div>
                        <div class="dual-label-row"><div><small>Min</small><span id="budget_pressure_low_label">1%</span></div><div><small>Max</small><span id="budget_pressure_high_label">5%</span></div></div>
                    </div>

                    <div class="range-card">
                        <label>Material shock impact (%)</label>
                        ...
                        <div class="dual-slider">
                            <div class="slider-track-bg"></div><div class="slider-track-fill" id="material_shock_fill"></div>
                            <input type="range" id="material_shock_low" min="0" max="20" value="1" oninput="updateDualRange('material_shock', '%', this)">
                            <input type="range" id="material_shock_high" min="0" max="20" value="4" oninput="updateDualRange('material_shock', '%', this)">
                        </div>
                        <div class="range-minmax"><span>0%</span><span>20%</span></div>
                        <div class="dual-label-row"><div><small>Min</small><span id="material_shock_low_label">1%</span></div><div><small>Max</small><span id="material_shock_high_label">4%</span></div></div>
                    </div>

                    <div class="range-card">
                        <label>Material inflation (%)</label>
                        ...
                        <div class="dual-slider">
                            <div class="slider-track-bg"></div><div class="slider-track-fill" id="material_inflation_fill"></div>
                            <input type="range" id="material_inflation_low" min="0" max="20" value="3" oninput="updateDualRange('material_inflation', '%', this)">
                            <input type="range" id="material_inflation_high" min="0" max="20" value="10" oninput="updateDualRange('material_inflation', '%', this)">
                        </div>
                        <div class="range-minmax"><span>0%</span><span>20%</span></div>
                        <div class="dual-label-row"><div><small>Min</small><span id="material_inflation_low_label">3%</span></div><div><small>Max</small><span id="material_inflation_high_label">10%</span></div></div>
                    </div>
                    ...
                    <div class="range-card">
                        ...
                        <div class="range-line">
                            <div class="single-range">
                                ...
                                <div class="range-minmax"><span>1%</span><span>30%</span></div>
                            </div>
                            <div class="value-pill"><span id="maxIncVal">20</span>%</div>
                        </div>
                        <small>Price increase is tested across this range. It is not randomized.</small>
                    </div>
                </div>
            </section>

            <section class="panel market-panel">
                <div class="section-head"><div class="section-num">5</div><h2>Monte Carlo Settings</h2></div>
                <div class="input-grid">
                    <div class="form-group" style="grid-column:1 / -1;">
                        <label>Number of simulations per price level</label>
                        <div class="mc-options">
                            <label class="mc-option"><input type="radio" name="n_simulations" value="1000" checked><span>1,000</span></label>
                            <label class="mc-option"><input type="radio" name="n_simulations" value="2000"><span>2,000</span></label>
                            <label class="mc-option"><input type="radio" name="n_simulations" value="3000"><span>3,000</span></label>
                            <label class="mc-option"><input type="radio" name="n_simulations" value="5000"><span>5,000</span></label>
                            <label class="mc-option"><input type="radio" name="n_simulations" value="10000"><span>10,000</span></label>
                            <label class="mc-option"><input type="radio" name="n_simulations" value="20000"><span>20,000</span></label>
                        </div>
                        <small>More simulations improve stability but take longer to compute, especially on free hosting.</small>
                    </div>
                </div>
                <div class="info-box">
                    <div class="info-icon">▣</div>
                    <div>
                        <b style="color:var(--green-2);">What happens next?</b>
                        <small>Monte Carlo simulation randomly samples market conditions within your selected ranges to estimate demand, cost, profit, and risk outcomes.</small>
                    </div>
                </div>
            </section>
        </div>
        ...
        <div class="bottom-bar">
            <div class="ready">
                ...
                <div><b style="color:var(--green-2);">Ready to simulate</b><small>All required inputs are available.</small></div>
            </div>
            <button class="run-btn" type="submit">▶ Run Simulation</button>
        </div>
    </form>
</div>
...
body {
    ...
    font-family: Poppins, Inter, Arial, sans-serif;
    background:
        radial-gradient(circle at 50% 22%, rgba(0,204,102,0.15), transparent 31%),
        radial-gradient(circle at 10% 88%, rgba(56,189,248,0.10), transparent 34%),
        radial-gradient(circle at 90% 86%, rgba(0,204,102,0.08), transparent 34%),
        linear-gradient(135deg, #071020, #0f172a 55%, #08111f);
    color: white;
    min-height: 100vh;
    display: flex;
    align-items: center;
...
.loading-card {
    ...
    border-radius: 24px;
    background: linear-gradient(180deg, rgba(18, 34, 58, .74), rgba(10, 22, 39, .84));
    border: 1px solid rgba(148,163,184,.18);
...
.loading-card::before {
    ...
    inset: -160px;
    background:
        radial-gradient(circle at 50% 18%, rgba(0,204,102,.20), transparent 28%),
        repeating-radial-gradient(circle at 50% 18%, rgba(255,255,255,.035) 0 1px, transparent 1px 38px);
    opacity: .86;
    pointer-events: none;
}
.sim-badge {
...
.pulse-dot {
    ...
    box-shadow: 0 0 18px rgba(38,232,137,.72);
...
.progress-fill {
    ...
    border-radius: 999px;
    background: linear-gradient(90deg, var(--green), var(--green-2));
    box-shadow: 0 0 28px rgba(0,204,102,.42);
...
.mini-status span.active-status { color: var(--green-2); }
.info-row {
    width: min(760px, 100%);
    margin: 34px auto 0;
    display: grid;
...
.tip b { color: var(--green-2); }
@media (max-width: 720px) {
    .loading-card { padding: 48px 20px 34px; }
    .progress-wrap, .info-row { grid-template-columns: 1fr; }
    .percent-small { text-align: center; }
    .mini-status { flex-direction: column; align-items: center; }
}
</style>
</head>
<body>
<div class="loading-shell">
    <div class="pp-brand"><span class="price-word">Price</span><span class="pilot-word">Pilot</span></div>
    <div class="tagline">Smarter pricing. Better results.</div>

    <div class="loading-card">
        <div class="sim-badge"><span class="pulse-dot"></span>Running Monte Carlo simulation</div>

        <h1 id="loadingTitle">Preparing data</h1>
        <p id="stageText">Checking input assumptions and preparing the simulation engine...</p>

        <div class="progress-wrap">
            <div class="progress-track"><div class="progress-fill" id="progressFill"></div></div>
            <div class="percent-small" id="progressPercent">0%</div>
        </div>

        <div class="mini-status">
            <span id="status0" class="active-status">Preparing data</span>
            <span id="status1">Estimating demand</span>
            <span id="status2">Running simulation</span>
            <span id="status3">Finalizing results</span>
        </div>
        ...
        <div class="info-row">
            <div class="info-box">
                <div class="info-icon">◷</div>
                <div>
                    <div class="info-title">Estimated time remaining</div>
                    <div class="info-value">~ <span id="timeLeft">{{ estimated_seconds }}</span> seconds</div>
                </div>
            </div>
            <div class="info-box">
                <div class="info-icon">💡</div>
                <div>
                    ...
                    <div class="did-text" id="factText">Monte Carlo methods were named after the Monte Carlo Casino because they use randomness to explore many possible outcomes.</div>
                </div>
            </div>
        </div>

        <div class="tip">✦ Tip: More simulations usually create a more stable recommendation, but they also take longer to compute.</div>
    </div>
</div>

<script>
const estimatedSeconds = Number({{ estimated_seconds }});
const estimatedMs = Math.max(4500, estimatedSeconds * 1000);
const stages = [
    {p: 4,  title: "Preparing data", text: "Checking input assumptions and preparing the simulation engine...", status: 0},
    {p: 24, title: "Estimating demand", text: "Estimating demand changes from elasticity and market ranges...", status: 1},
    {p: 55, title: "Running simulation", text: "Sampling market conditions and calculating cost and profit outcomes...", status: 2},
    {p: 82, title: "Comparing scenarios", text: "Comparing pricing outcomes and downside risk...", status: 3},
    {p: 96, title: "Generating recommendation", text: "Finalizing personalized recommendation and result dashboard...", status: 3},
    {p: 100, title: "Opening dashboard", text: "Opening your PricePilot result dashboard...", status: 3}
];
const facts = [
    "The word 'robot' comes from a Czech word meaning forced labor.",
    "Aluminum is one of the most recycled metals in the world.",
    "The first spreadsheet software, VisiCalc, helped popularize personal computers in business.",
    "Monte Carlo simulation is widely used in finance, engineering, logistics, and risk management.",
    "In pricing, a small margin change can create a large profit impact when order volume is high.",
    "Steel is one of the most important materials for machinery, construction, and manufacturing supply chains.",
    "A dashboard is useful because it turns many raw calculations into a decision users can understand quickly."
];
...
const computePromise = fetch("/compute", {method: "POST"})
    .then(function(response) {
        if (!response.ok) throw new Error("Compute request failed");
    ...
    .catch(function(error) {
        computeFailed = true;
        document.getElementById("loadingTitle").innerText = "Simulation failed";
        document.getElementById("stageText").innerText = "Please go back to the input page and try again.";
        console.error(error);
    });
...
function setStage(progress) {
    ...
    for (const s of stages) {
        if (progress >= s.p) stage = s;
    }
    document.getElementById("loadingTitle").innerText = stage.title;
    document.getElementById("stageText").innerText = stage.text;
    for (let i = 0; i < 4; i++) {
        const item = document.getElementById("status" + i);
        item.classList.toggle("active-status", i === stage.status);
    }
}
...
function updateProgress() {
    ...
    const timedProgress = Math.round((elapsed / estimatedMs) * 100);

    let progress;
    if (computeDone && elapsed >= estimatedMs) {
        progress = 100;
    } else if (elapsed >= estimatedMs) {
        // If the visual timer finishes but backend is still computing, wait near the end.
        progress = 99;
    } else {
        // Keep loading smooth even if the backend finishes early.
        // The page only opens after the bar naturally reaches 100%.
        progress = Math.min(timedProgress, 99);
    }

    document.getElementById("progressFill").style.width = progress + "%";
    document.getElementById("progressPercent").innerText = progress + "%";

    const remainingByEstimate = Math.ceil((estimatedMs - elapsed) / 1000);
    document.getElementById("timeLeft").innerText = progress >= 100 ? "0" : Math.max(1, remainingByEstimate);
    setStage(progress);

    if (progress >= 100 && computeDone) {
        setTimeout(function(){ window.location.href = "/result"; }, 450);
        return;
    }
...
.final-run-btn {
    ...
    background: linear-gradient(135deg, rgba(239,68,68,0.28), rgba(190,24,93,0.22));
    color: #ffe4e6;
    font-weight: 950;
...
.final-run-btn:hover {
    ...
    background: linear-gradient(135deg, rgba(239,68,68,0.40), rgba(190,24,93,0.32));
}
...
<div class="dashboard-shell">
    ...
    <div class="topbar">
        <div class="brand-row">
            ...
            <div class="title-wrap">
                <h1><span class="pp-brand"><span class="price-word">Price</span><span class="pilot-word">Pilot</span></span> Result</h1>
        ...
        <div class="top-actions">
            ...
            <button class="pill-btn" onclick="window.print()">🖨 Print / Save as PDF</button>
        </div>
    </div>

    <div class="main-grid">
        <section class="card hero-card unified-recommendation-card">
            <div class="hero-content">
                <div class="target-ring">✓</div>
                <div>
                    <div class="eyebrow">{{ result.recommendation_label }}</div>
                    <h2>Increase price by <span>{{ result.recommended_range }}</span></h2>
                    <p>{{ result.recommendation_subtitle }}</p>
                    <div class="confidence-strip unified-strip">● {{ result.recommendation_strip }}</div>
                </div>
            </div>
            <div class="hero-metrics unified-metrics">
                <div class="hero-mini">
                ...
                <div class="hero-mini">
                    ...
                    <div class="value">{{ result.recommended_profit }} VND</div>
                    <div class="sub">{{ result.profit_improvement }} vs. current</div>
                </div>
                <div class="hero-mini">
                ...
                <div class="hero-mini">
                    ...
                    <div class="value">{{ result.recommended_loss_probability }}</div>
                    <div class="sub">downside risk</div>
                </div>
                <div class="hero-mini">
    ...
    <section class="analytics-grid" style="margin-top:16px; grid-template-columns: 1fr 1fr;">
        ...
        <div class="card insights-card">
            ...
            <div class="insight-row" style="grid-template-columns: 1fr;">
                <div class="insight-item"><div class="icon">↗</div><h4>{{ result.quick_profit_title }}</h4><p>{{ result.quick_profit_text }}</p></div>
    ...
    <section class="strategy-grid">
        <div class="card strategy-card aggressive-card">
            ...
            <div class="strategy-meta">
                ...
                <div class="strategy-stat"><span>{{ result.aggressive_profit_label }}</span><strong>{{ result.aggressive_profit_improvement }}</strong></div>
        ...
        <div class="card strategy-card balanced-card recommended">
            ...
            <div class="strategy-meta">
                ...
                <div class="strategy-stat"><span>{{ result.balanced_profit_label }}</span><strong>{{ result.profit_improvement }}</strong></div>
        ...
        <div class="card strategy-card conservative-card">
            ...
            <div class="strategy-meta">
                ...
                <div class="strategy-stat"><span>{{ result.conservative_profit_label }}</span><strong>{{ result.conservative_profit_improvement }}</strong></div>
    ...
    <div class="final-run-section">
        <a class="final-run-btn" href="/input">↻ Run Again</a>
    </div>
</div>
...
def loading():
    ...
    try:
        n_simulations = int(form.get("n_simulations", 1000))
        max_price_increase = float(form.get("max_price_increase", 20))
        price_levels = int(max_price_increase / 0.5) + 1
        workload = max(n_simulations * price_levels, 1)
        estimated_seconds = int(min(max(5, workload / 7000), 60))
    except Exception:
        estimated_seconds = 8
    return render_template_string(LOADING_HTML, estimated_seconds=estimated_seconds)
...
def build_strategy_text(strategy_name, price_range, selected_inc, expected_profit, profit_improvement_pct,
    ...
    if strategy_name == "aggressive":
        ...
        if profit_level == "negative":
            paragraphs.append(
                f"This range is the <b>main recommendation</b> because it is the best defensive option within the tested assumptions. For this business, the selected increase is around <b>{selected_inc:.1f}%</b>."
            )
        else:
            paragraphs.append(
        ...
        if profit_level == "strong":
            ...
            paragraphs.append(
                f"The expected profit change is negative at <b>{profit_improvement_pct:.1f}%</b>. Under these assumptions, this should be treated as a defensive adjustment rather than a fully profit-improving price increase."
            )
...
def build_personalized_insight(recommended_range, recommended_inc, current_profit, recommended_profit,
    ...
    if profit_level == "negative":
        ...
        profit_explain = "The recommended point does not beat the current profit baseline. Under these assumptions, the recommendation should be interpreted as a defensive adjustment rather than a normal profit-growth signal."
    elif profit_level == "small":
    ...
    # Build the implementation-plan wording from the actual scenario outputs.
    if profit_level == "negative":
        plan_positioning = (
            "The balanced range is used as a defensive starting point because no tested range fully improves profit under the current assumptions."
        )
    elif risk_level in ["elevated", "high"] or demand_level in ["significant", "high"]:
        plan_positioning = (
            "The balanced range is used as a controlled starting point because profit improves, but demand loss or downside risk still requires a cautious rollout."
        )
    else:
        plan_positioning = (
            "The balanced range is used as the starting point because it improves profit while keeping demand loss and downside risk at a manageable level."
        )

    if elasticity_level in ["high", "very high"]:
        elasticity_plan_note = (
            "Because historical elasticity is relatively high, larger price jumps should be avoided until customer response is observed."
        )
    else:
        elasticity_plan_note = (
            "Historical elasticity is not extreme, but customer response should still be monitored before moving toward the upper end of the tested ranges."
        )

    if avg_material_inflation_pct >= 8:
        cost_plan_note = (
            "Material inflation is high enough that actual input costs should be reviewed frequently while the new price is being introduced."
        )
    else:
        cost_plan_note = (
            "Material inflation is included because even moderate input-cost increases can reduce margin if prices are left unchanged for too long."
        )

    if profit_level == "negative":
        defensive_market_note_html = (
            "<p><b>Defensive interpretation:</b> Expected profit remains below the current baseline under these assumptions. "
            "The recommended range should therefore be read as a defensive adjustment: it is the most suitable tested range under the simulated pressure, "
            "not a signal that the firm has found a fully profit-improving scenario.</p>"
        )
    else:
        defensive_market_note_html = ""
    ...
    <div class="personalized-block">
        <h3>3. Market Condition Analysis</h3>
        <p>
            Average market growth contributes about <b>{avg_market_growth_pct:.1f}%</b> support to demand in the simulation.
            At the same time, customer budget pressure reduces demand by about <b>{avg_budget_pressure_pct:.1f}%</b>,
            while material shock impact reduces demand by about <b>{avg_material_shock_pct:.1f}%</b>.
        </p>
        <p>
            Before applying price elasticity, the net market effect is approximately
            <b>{net_market_effect_pct:.1f}%</b>
            (= market growth minus customer budget pressure and material shock impact). This separates market-driven demand movement
            from the additional demand change caused by the firm's own price increase.
        </p>
        <p>
            Material inflation averages about <b>{avg_material_inflation_pct:.1f}%</b>. This variable increases variable cost per order,
            so it affects margin and profit through the cost side. It is not treated as a direct demand-side shock in the model.
        </p>
        {defensive_market_note_html}
    </div>

    <div class="personalized-block">
        <h3>4. Why this recommendation was selected</h3>
        <p>{profit_explain}</p>
        <p>{demand_explain}</p>
        <p>{risk_explain}</p>
        <p>
    ...
    <div class="personalized-block">
        <h3>5. Main pressure driver behind demand change</h3>
        <p>
    ...
    <div class="personalized-block">
        <h3>6. Customer sensitivity and cost structure diagnosis</h3>
    ...
    <div class="personalized-block">
        ...
        <p>
            The starting point is the <b>{recommended_range}</b> balanced range because it gives the best trade-off between expected profit, order retention, and downside risk in this simulation.
            Expected demand loss is <b>{recommended_demand_loss_pct:.1f}%</b>, while the probability of profit ending below the current level is <b>{recommended_risk_pct:.1f}%</b>.
            These two results determine how quickly the price change should be introduced. {plan_positioning}
        </p>
        <p>
            Historical price elasticity is <b>{elasticity:.2f}</b>, so demand is expected to react when prices change. Estimated material inflation of <b>{avg_material_inflation_pct:.1f}%</b> also supports a price adjustment because input-cost pressure can reduce margin even when order volume is stable.
            {elasticity_plan_note} {cost_plan_note}
        </p>
        <p><b>Recommended next steps:</b></p>
        <ol>
            <li><b>Begin at the lower or middle part of the balanced range.</b> Use the new price first for new quotations, selected repeat customers, or orders where quality, lead time, and delivery reliability matter more than the lowest price.</li>
            <li><b>Track the indicators that correspond to the simulation.</b> Monitor quotation acceptance, monthly order volume, and actual material cost per order. These show whether demand, risk, and cost are moving close to the assumptions used in the model.</li>
            <li><b>Compare actual demand with the simulated demand loss.</b> The model expects demand to fall by about <b>{recommended_demand_loss_pct:.1f}%</b>. If order volume and quotation acceptance stay broadly in line with that result while material costs remain elevated, the upper part of the balanced range can be tested.</li>
            <li><b>Use the scenarios as decision boundaries.</b> Move closer to the conservative range if orders or quotation acceptance fall materially faster than expected. Consider the aggressive range only after demand remains stable and the initial price adjustment performs close to the model's expected outcome.</li>
        </ol>
        <p>
            This follow-up turns the simulation into a practical pricing process. Monte Carlo provides a range of likely outcomes; actual quotation acceptance, orders, and material costs show which outcome is unfolding and guide the next adjustment.
        </p>
    </div>
    """


def compute_result_context(form):
    ...
    n_simulations = max(1, min(n_simulations, 20000))
    ...
    for inc in price_grid:
        ...
        # Vectorized simulation keeps memory low on Render Free.
        # It avoids storing every individual simulation row in a giant DataFrame.
        market_growth_sims = np.random.uniform(market_growth_min, market_growth_max, n_simulations)
        budget_pressure_sims = np.random.uniform(budget_pressure_min, budget_pressure_max, n_simulations)
        material_shock_sims = np.random.uniform(material_shock_min, material_shock_max, n_simulations)
        material_inflation_sims = np.random.uniform(material_inflation_min, material_inflation_max, n_simulations)

        demand_changes_array = (
            market_growth_sims
            - budget_pressure_sims
            - material_shock_sims
            - elasticity * inc_rate
        )

        demands_array = np.maximum(base_demand * (1 + demand_changes_array), 0.1)
        new_selling_price = base_price * (1 + inc_rate)

        new_variable_cost_per_order = variable_cost_per_order * (1 + material_inflation_sims)
        new_fixed_cost_per_order = fixed_cost / np.maximum(demands_array, 1)
        costs_array = new_variable_cost_per_order + new_fixed_cost_per_order

        profits_array = (new_selling_price - costs_array) * demands_array

        counts, edges = np.histogram(profits_array, bins=32)
        probabilities = counts / max(counts.sum(), 1) * 100
        centers = (edges[:-1] + edges[1:]) / 2
        monte_carlo_payload[f"{float(inc):.1f}"] = {
            "bin_centers": [float(x) for x in centers],
            "probabilities": [float(y) for y in probabilities],
            "mean_profit": float(np.mean(profits_array)),
            "risk_pct": float(np.mean(profits_array < current_profit_value_for_dist) * 100),
            "average_demand": float(np.mean(demands_array)),
    ...
    # For market insight, the expected average of each uniform range is the midpoint.
    # This avoids building a massive simulation DataFrame while keeping the analysis consistent.
    avg_market_growth_pct = ((market_growth_min + market_growth_max) / 2) * 100
    avg_budget_pressure_pct = ((budget_pressure_min + budget_pressure_max) / 2) * 100
    avg_material_shock_pct = ((material_shock_min + material_shock_max) / 2) * 100
    avg_material_inflation_pct = ((material_inflation_min + material_inflation_max) / 2) * 100
    ...
    # The interactive distribution modal uses monte_carlo_json.
    # dist_chart is kept only for compatibility with the render context.
    dist_chart = ""

    def profit_label_for(value):
        return "Profit gap vs current" if value < 0 else "Profit improvement"

    if balanced_profit_improvement_pct < 0:
        recommendation_label = "Defensive Recommendation"
        recommendation_subtitle = "No fully profitable scenario found under current assumptions. This range is a defensive adjustment under the simulated market pressure."
        recommendation_strip = "Defensive adjustment based on profit, demand, and downside risk"
        quick_profit_title = "Defensive adjustment"
        quick_profit_text = (
            f"Expected profit remains below the current baseline by {abs(balanced_profit_improvement_pct):.1f}%. "
            "The recommended range is selected as a defensive adjustment under the simulated market pressure, not as a fully profit-improving scenario."
        )
    else:
        recommendation_label = "Top Recommendation"
        recommendation_subtitle = "This balanced range is selected because it improves profit while keeping demand loss and downside risk within acceptable levels for this business."
        recommendation_strip = "Balanced recommendation based on profit, demand, and downside risk"
        quick_profit_title = "Profit opportunity"
        quick_profit_text = (
            f"Balanced strategy improves expected profit by {balanced_profit_improvement_pct:.1f}% "
            "while avoiding the most aggressive range."
        )
    ...
    result = {
        ...
        "recommendation_strip": recommendation_strip,
        "quick_profit_title": quick_profit_title,
        "quick_profit_text": quick_profit_text,
        ...
        "aggressive_profit_label": profit_label_for(aggressive_profit_improvement_pct),
        ...
        "balanced_profit_label": profit_label_for(balanced_profit_improvement_pct),
        ...
        "conservative_profit_label": profit_label_for(conservative_profit_improvement_pct),
...
def compute():
    ...
    if not form or not job_id:
        return {"ok": False, "error": "Missing simulation inputs."}, 400
    try:
        RESULT_CACHE[job_id] = compute_result_context(form)
        return {"ok": True}
...
def result_page():
    ...
    context = RESULT_CACHE.get(job_id)
    if context is None:
        # Fallback: if the user refreshes /result directly, compute once synchronously.
        context = compute_result_context(form)
        if job_id:
            RESULT_CACHE[job_id] = context

    return render_template_string(
        RESULT_HTML,
        result=context["result"],
        profit_chart=context["profit_chart"],
        demand_chart=context["demand_chart"],
        dist_chart=context["dist_chart"],
        monte_carlo_json=context["monte_carlo_json"]
    )
